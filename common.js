...
(function(){
  ...
  var P={
    ...
    'salary':{k:'Regression · Managerial Decision Making',t:'Data-Role Salary Prediction',
      f:'A multiple-regression model on a <strong>930-observation</strong> global dataset of data-role salaries, built in BUAL 5380 (Managerial Decision Making) at Lamar University — <strong>backward elimination</strong> to drop weak predictors, reaching <strong>R² ≈ 71%</strong>, then validated with a project partner for salary-benchmarking decisions. I rebuilt the same method as a <strong>runnable, open-data pipeline</strong>: it fetches a public salaries dataset and finds that experience level and a US-based employer dominate pay (EX +$88k, US +$59k), while <strong>remote ratio doesn’t predict salary at all</strong> — the model drops it first. It reports an honest R² ≈ 0.39, because R² is a property of the data and features, not a fixed score.',
      chips:[{b:'R² ≈ 71%',s:'coursework model'},{b:'930',s:'observations'},{b:'Reproducible',s:'open-data pipeline'}],
      links:[{h:'https://github.com/muradhasib01/mis-graduate-portfolio/tree/main/salary-regression',t:'Run the pipeline ↗'}]},
    ...
    'research':{k:'Research · Graph Neural Networks',t:'GNN-Based Fraud Detection',
      f:'Co-authored an academic paper applying <strong>graph neural networks (GNNs)</strong> to fraud detection — modeling transactions as a graph, so the method learns from how entities connect rather than from isolated features alone. The paper has been <strong>cited 46 times</strong>, and the research spanned AI/ML, cybersecurity, blockchain, and enterprise data systems.',
      chips:[{b:'Cited 46×',s:'this paper'},{b:'104',s:'total citations'},{b:'GNN',s:'graph neural networks'}],
      links:[{h:'https://scholar.google.com/citations?user=PRL0PiQAAAAJ&hl=en',t:'Google Scholar ↗'}]},
